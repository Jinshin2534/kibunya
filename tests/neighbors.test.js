...
describe('similarDays', () => {
  ...
  it('完全に距離が同じとき順序は元の配列順', () => {
    const same = [
      { date: 'a', z: z(0) },
      { date: 'b', z: z(0) },
      { date: 'c', z: z(0) },
    ]
    const r = similarDays(z(0), same, 3)
    // 同じ距離（0）なら元の順序を保つ
    expect(r.map((x) => x.entry.date)).toEqual(['a', 'b', 'c'])
  })
  ...
  it('excludeDate がすべてのエントリを除外するとき空を返す', () => {
  ...
  it('複数のエントリを除外したとき残りだけ返す', () => {
    const r1 = similarDays(z(0), entries, 10, '2026-01-01')
    const r2 = similarDays(z(0), entries, 10, '2026-01-02')
    const dates1 = r1.map((x) => x.entry.date)
    const dates2 = r2.map((x) => x.entry.date)
    expect(dates1).not.toContain('2026-01-01')
    expect(dates2).not.toContain('2026-01-02')
  })
})
